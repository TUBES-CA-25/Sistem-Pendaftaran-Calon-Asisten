                ...
                <tr>
                    ...
                    <td>
                        <div style="display: flex; gap:5%;">
                            ...
                            <img src="\u003c?= BASE_URL ?\u003e/Assets/Img/delete.svg" alt="delete" class="delete-button"
                                data-id="<?= $ruangan['id'] ?>" style="cursor: pointer;">
                        </div>
                    </td>
                </tr>
            ...
            $.ajax({
                ...
                success: function (response) {
                    if (response.status === 'success') {
                        showModal('Ruangan berhasil ditambahkan!', '\u003c?= BASE_URL ?\u003e/Assets/gif/success.gif');
                        document.querySelector('a[data-page="ruangan"]').click();
                        $('#tambahRuanganModal').modal('hide');
                    } else {
                        showModal('Ruangan gagal ditambahkan: ' + response.message, '\u003c?= BASE_URL ?\u003e/Assets/gif/error.gif');
                    }
                },
        ...
        $('.delete-button').on('click', function () {
            ...
            showConfirm('Apakah Anda yakin ingin menghapus ruangan ini?', function () {
                $.ajax({
                    ...
                    success: function (response) {
                        if (response.status === 'success') {
                            showModal('Ruangan berhasil dihapus!', '\u003c?= BASE_URL ?\u003e/Assets/gif/success.gif');
                            document.querySelector('a[data-page="ruangan"]').click();
                        } else {
                            showModal('Gagal menghapus ruangan: ' + response.message, '\u003c?= BASE_URL ?\u003e/Assets/gif/error.gif');
                        }
                    },
                    error: function (xhr, status, error) {
            ...
            $.ajax({
                ...
                success: function (response) {
                    if (response.status === 'success') {
                        ...
                        showModal('Gagal memperbarui ruangan: ' + response.message, '\u003c?= BASE_URL ?\u003e/Assets/gif/failed.gif');
                    }
                },
                error: function (xhr, status, error) {
                    console.error('Error:', error);
