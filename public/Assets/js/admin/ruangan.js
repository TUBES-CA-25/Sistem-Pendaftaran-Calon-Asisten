/**
 * ruangan.js — Vanilla JS (no jQuery dependency)
...
(function () {
    ...
    function initRoomsScript() {
        ...
        // showDetailView() dihapus: Fitur detail ruangan tidak pernah selesai: #ruanganDetailSection dan #detailRoomTitle tidak ada di markup, dan tidak ada tombol pemanggil.
        ...
        // ── TABS ─────────────────────────────────
        // Blok listener tab dihapus: memakai event Bootstrap 'shown.bs.tab'
        // (Bootstrap sudah dibuang) DAN merujuk #pills-presentasi-tab /
        // #pills-testulis-tab / #pills-wawancara-tab yang tidak ada di satu
        // pun view; .nav-link juga nol di view ruangan, jadi loop tidak pernah
        // berjalan. Satu rumpun dengan showDetailView() di atas: sisa fitur
        // "detail ruangan" yang tidak pernah selesai.
        //
        // currentType dipertahankan (dipakai loadParticipants dan penentuan
        // kolom status) dan kini tetap pada nilai default 'tes_tulis'.
                                ...
                                <td class="py-4 px-4">
                                    <div class="flex items-center gap-3">
                                        <div class="w-12 h-12 bg-blue-50 text-blue-600 rounded-lg flex items-center justify-center shrink-0 border border-blue-100 overflow-hidden">
                                            <a href="${APP_URL.replace('/public', '')}/res/ruangan_lab/${res.gambar || 'default_room.png'}" target="_blank" title="Klik untuk memperbesar gambar" class="block w-full h-full">
                                                <img src="${APP_URL.replace('/public', '')}/res/ruangan_lab/${res.gambar || 'default_room.png'}" alt="Gambar Lab" class="w-full h-full object-cover hover:scale-110 transition-transform duration-300">
                                            </a>
                                        </div>
                                        <div>
                    ...
                    if (res.status === 'success') {
                        ...
                        if (row) {
                            ...
                            if (res.gambar) {
                                ...
                                if (imgEl) imgEl.src = `${APP_URL.replace('/public', '')}/res/ruangan_lab/${res.gambar}`;
                                if (aEl) aEl.href = `${APP_URL.replace('/public', '')}/res/ruangan_lab/${res.gambar}`;
                            }
    ...
    if (!window._roomsDocBound) {
        document.addEventListener('click', function (e) {
            ...
            if (editBtn) {
                ...
                var modal = UI.modal.ref(document.getElementById('updateRuanganModal'));
                modal.show();
                return;
            }
