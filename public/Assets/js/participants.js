...
(function () {
    const initDaftarPesertaScript = function () {
        ...
        $('#addNotificationForm').off('submit').on('submit', function (e) {
            ...
            // Button is outside form with form="addNotificationForm" attribute
            ...
            fetch(`${APP_URL}/addallnotif`, {
                ...
                .then(data => {
                    ...
                    if (data.status === 'success') {
                        ...
                        console.warn('Partial success:', data.errors);
                        showSuccessPopup(data.message);
                        ...
                        if (typeof showErrorPopup === 'function') {
                            showErrorPopup(data.message || 'Gagal mengirim notifikasi');
                        } else {
                            showAlert(data.message || 'Gagal mengirim notifikasi', false);
                        }
                    }
                })
                .catch(error => {
                    console.error('Error:', error);
                    if (typeof showErrorPopup === 'function') {
                        showErrorPopup('Terjadi kesalahan saat mengirim notifikasi');
                    } else {
